...
void greedy(canh ds_canh[], int n, canh PA[]){
	...
	int r_dau, r_cuoi;
	//Chon cac canh nho nhaats khoong tao thanh dinh cap 3 va chu trinh de dua vao PA
	j = 0;
	for(i = 0; i < n*(n-1)/2 && j < n-1; i++){
		...
		if(!dinh_cap3(PA, j, ds_canh[i]) && !chu_trinh(r_dau, r_cuoi) ){
			PA[j] = ds_canh[i]; //ds canh khoong tao dinh cap 3 va chu trinh thieu dua vao PA
			j++;
